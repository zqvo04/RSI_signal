# RSI·MACD 크로스 텔레그램 알림봇

OKX USDT 무기한 선물의 RSI(14) 반전 돌파와 MACD(12, 26, 9) 골든/데드크로스를 감지해 Telegram으로 알려주는 GitHub Actions 기반 알림봇입니다. 주문을 실행하지 않으며, 공개 시세 데이터만 조회합니다.
...
API 데이터의 결측 행은 지표 계산 전에 제거합니다. 또한 진행 중인 캔들은 사용하지 않습니다. 캔들의 종료 시각에 30초의 여유를 둔 뒤, 완전히 확정된 최근 두 캔들만 비교합니다. 따라서 1시간·4시간 캔들에서 마감 전 일시적인 돌파가 알림으로 발생하지 않습니다.
...
**RSI:**
```
🚨 [1h] 신호 발생 (중요도: ⚡️ 중간 신뢰도)
- 코인: BTC
- 포지션: 📈 LONG
- 현재가: 65000 USDT
- RSI 지표: 28.50 -> 31.20 (돌파 완료)
```

**MACD:**
```
🚨 [4h] MACD 신호 발생 (중요도: 🔥 높은 신뢰도)
- 코인: ETH
- 포지션: 📈 LONG (골든크로스)
- 현재가: 3500 USDT
- MACD 지표: DIF -0.3000 / DEA -0.2500 -> DIF -0.1000 / DEA -0.1500 (돌파 완료)
```
...
이를 막기 위해 **돌파 캔들이 방금 닫힌 직후 첫 실행에서만** 알림을 보냅니다. 구체적으로, 돌파 캔들의 종료 시각이 직전 스캔 주기(15분) 이내일 때만 "신선한 신호"로 간주합니다(`is_freshly_closed`). GitHub Actions 실행은 상태를 유지하지 않으므로, 별도의 저장소 없이 시간 창(time window)만으로 중복을 제거합니다. RSI와 MACD 모두 동일한 규칙을 적용합니다.
...
한 번의 실행(`main.py`의 `main`)은 감시 대상 15개 코인에 대해 RSI 45건 + MACD 30건, 총 **75건**을 다음 순서로 처리합니다.
...
3. **공통 처리**
   ...
   - **알림 전송**: 토큰·채팅 ID는 공백을 제거해 사용하며, Telegram이 오류를 반환하면 응답 본문(예: `chat not found`)을 로그에 남겨 원인을 확인할 수 있게 합니다(`send_telegram_message`).
   - **속도 제한**: 각 검사 사이에 짧은 지연(`REQUEST_DELAY_SECONDS`)을 둬 OKX·Telegram 엔드포인트 부하를 줄입니다.
   - **실행 요약**: 검사 건수·신호 수·오류 수와 BTC의 최근 확정 캔들 RSI·MACD 값을 GitHub Actions Summary에 기록합니다(`write_workflow_summary`). 모든 검사가 실패하면 워크플로우를 실패로 종료합니다.
...
Summary에는 검사 건수, 발생 신호 수, 오류 수가 표시됩니다. 정상 환경에서는 RSI 45건 + MACD 30건, 총 **75건**을 검사합니다. Telegram은 실제 LONG/SHORT 신호가 발생했을 때만 전송됩니다.
